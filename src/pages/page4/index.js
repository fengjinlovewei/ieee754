...
export default () => {
  ...
  const validation = (iee754) => {
    ...
    if (reg.test(iee754) && Exponent < 11111111111) {
      ...
      const { BinaryTruthValue, DecimalTruthValue } = IEEE754ToDecimal({ Sign, Exponent, Mantissa })
      return {
        Sign,
        Exponent,
        Hide: '1.',
      ...
      <div>
        {
          text.map((item, i) => {
            return (
              <span style={{ display: 'inline-block' }} key={item.size}>{i !== 0 && ` + `}
    ...
    <div>
      <div className="center">
        <div className="search-box4 center">
          ...
          <Button type="primary" onClick={enCode} style={{marginLeft:'20px'}}>编码</Button>
        </div>
      </div>
      {
        bitMap.map(item => (
